...
public class ItemHandler implements Handler {
	...
	public void delete(Integer id) {
		new ResourceDAO().deleteByPrimaryKey(id);
	}
	...
	public ObjectAdapter update(ObjectAdapter adapter) {
		if (adapter.getType() == ResourceTypeConst.TYPE_VERSION) {
			...
			Resource resource = adapter.getResource();
			(new ResourceDAO()).updateResource(resource);
			adapter.setPublicTime(resource.getCreateTime());
			adapter.setEditTime(resource.getRecentEditTime());
		}
		return adapter;
	}
	...
	public ObjectAdapter getObject(Integer id) {
		Resource resource = new ResourceDAO().findByPrimaryKey(id);
		if (resource == null) {
			return null;
		}
		ObjectAdapter adapter = new ObjectAdapter(resource);
		return adapter;
	}
	...
	public List<Item> selects(Integer page, Integer size) {
		return new ItemDAO().selects(page, size);
	}
