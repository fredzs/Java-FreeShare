...
public class PreAddtoListAction extends BaseAction {
	...
	public String execute(){		
		...
		for (Integer listId : latestList){
			Lists list =  new ListHandler().getObject(listId).getList();
			lists.add(new ListBrief(listId,	list.getName()));
			logger.info("listId:" + listId +",listName:" + list.getName());
		}
		return SUCCESS;		
	}
	...
	public String myList(){
		...
		tags += " AND " + TagValuesConst.FMT_LIST;
		//获取列表信息
		SrhResult srhResult = SearchManager.searchItems(NULL_QUERY, PAGE, MAX_SIZE, tags);
		if(srhResult == null){
			return SUCCESS;
		}
