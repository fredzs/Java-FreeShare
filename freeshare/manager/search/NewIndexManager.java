...
public class NewIndexManager implements IndexManagerFactory {
	 ...
	 *            要添加的标签
	 */
	public boolean addIndex(ObjectAdapter adapter) {
		logger.info("创建资源索引。");
		String content = new BuildContentFactory("search").produce().buildContent(adapter);
		String url = (adapter.getType() == -1) ? ("list?id=" + adapter
				.getId()) : ("item?id=" + adapter.getId());
		IndexUtil iu = IndexUtil.getInstance();
		List<String> tags = iu.getTags(adapter); //资源的固有属性
		addIndex(url, adapter.getName(), content, tags);
	...
	public static void main(String[] args){
		...
		list.add("LABLES:演示");
		new NewIndexManager().removeTags("item?id=877", list);
	}
