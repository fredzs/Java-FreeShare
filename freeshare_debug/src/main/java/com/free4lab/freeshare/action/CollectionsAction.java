package com.free4lab.freeshare.action;

import java.util.LinkedList;
import java.util.List;

import org.apache.log4j.Logger;
import org.apache.struts2.ServletActionContext;

import com.free4lab.freeshare.handler.ItemHandler;
import com.free4lab.freeshare.handler.ListHandler;
import com.free4lab.freeshare.model.dao.Item;
import com.free4lab.freeshare.model.dao.Lists;
import com.free4lab.freeshare.model.dao.UserCollection;
import com.free4lab.freeshare.model.dao.UserCollectionDAO;

/**
 * TODO 重构成ObjectAdapter
 * @author zhaowei 添加收藏/查看我的收藏/删除某个收藏
 * 
 */
public class CollectionsAction extends BaseAction {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger
			.getLogger(CollectionsAction.class);
	private Integer id;
	private String type;

	// private Integer page_num;
	private Integer page; // 搜索第几页
	private Integer lastPage = 6;
	private final static Integer SIZE = 10;// 搜索结果一页呈现多少个结果
	private List<Object> myCtionList;
	public List<UserCollection> getUserCollections() {
		return userCollections;
	}

	public void setUserCollections(List<UserCollection> userCollections) {
		this.userCollections = userCollections;
	}

	private List<UserCollection> userCollections ;
	public String execute() {
		if (type == null) {
			setType("item");
		}
		if (new UserCollectionDAO().isExist(getSessionUID(), id, type)) {
			new UserCollectionDAO().create(getSessionUID(), id, type);
		}
		return SUCCESS;
	}

	public String collections() {
		logger.info("查看我的收藏,资源类型为：" + type);
		if(page == null){
			page = 1;
		}
		System.out
				.println("In the loginfilter, the request.getRequestURL() is "
						+ ServletActionContext.getRequest().getParameter(
								"__cleanup_recursion_counter"));
		if (type == null || type == "") {
			setType("item");
		}
		userCollections = new UserCollectionDAO()
				.findByUid(getSessionUID(), type);
		myCtionList = new LinkedList<Object>();

		if (userCollections != null && userCollections.size() != 0) {
			Integer PAGE = getPage() - 1;
			Integer size = PAGE*SIZE;
			for(int i = size ; i < size + 10 && i < userCollections.size(); i ++){
				UserCollection uc = userCollections.get(i);
				try {
					if (type.equals("item")) {
						Item item = new ItemHandler().getObject(uc.getIid()).getItem();
						if (item != null) {
							String description = delhtml(item.getDescription());
							if (description.length() > 50) {
								description = description.substring(0, 49)
										+ "...";
							}
							item.setDescription(description);
							myCtionList.add(item);
						}
					} else {
						Lists list = new ListHandler().getObject(uc.getIid()).getList();
						if (list != null) {
							String description = delhtml(list.getDescription());
							if (description.length() > 50) {
								description = description.substring(0, 49)
										+ "...";
							}
							list.setDescription(description);
							myCtionList.add(list);
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
					continue;
				}
			}
//			for (UserCollection uc : userCollections) {
//				try {
//					if (type.equals("item")) {
//						Item item = new ItemManager().get(uc.getIid());
//						if (item != null) {
//							String description = delhtml(item.getDescription());
//							if (description.length() > 50) {
//								description = description.substring(0, 49)
//										+ "...";
//							}
//							item.setDescription(description);
//							myCtionList.add(item);
//						}
//					} else {
//						Lists list = new ListManager().get(uc.getIid());
//						if (list != null) {
//							String description = delhtml(list.getDescription());
//							if (description.length() > 50) {
//								description = description.substring(0, 49)
//										+ "...";
//							}
//							list.setDescription(description);
//							myCtionList.add(list);
//						}
//					}
//				} catch (Exception e) {
//					e.printStackTrace();
//					continue;
//				}

//			}
		} 	
		if (null == myCtionList) {
			logger.info("您的收藏为空");
			setLastPage(1);
		} else {
			if (userCollections.size() % SIZE == 0) {
				setLastPage(userCollections.size() / SIZE);
			} else
				setLastPage(userCollections.size() / SIZE + 1);
		}
		return SUCCESS;
	}

	public String deleteColl() {
		logger.info("删除我的收藏,资源类型为：" + type);
		if (type == null || type == "") {
			setType("item");
		}
		new UserCollectionDAO().delete(getSessionUID(), id, type);
		return SUCCESS;
	}

	public Integer getId() {
		return id;
	}

	public void setId(Integer id) {
		this.id = id;
	}

	public String getType() {
		return type;
	}

	public void setType(String type) {
		this.type = type;
	}

	public Integer getLastPage() {
		return lastPage;
	}

	public void setLastPage(Integer lastPage) {
		this.lastPage = lastPage;
	}

	public Integer getPage() {
		return page;
	}

	public void setPage(Integer page) {
		this.page = page;
	}

	public List<Object> getMyCtionList() {
		return myCtionList;
	}

	public void setMyCtionList(List<Object> myCtionList) {
		this.myCtionList = myCtionList;
	}

}
