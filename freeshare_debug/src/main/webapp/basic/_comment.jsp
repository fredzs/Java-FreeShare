...
<s:if test = "#session.userId != null">
	...
	<div id="cmtarea_<s:property value="id"/>_<s:property value="resourceType"/>">
		<s:iterator value="docs" id="doc" status="st">
		<div>
			...
			<table>
				...
				<td rowspan="2" width="60" class="centeralign topveralign">
					...
					<a href="javascript:void(0)" class="grayletter" onclick="quickCommunicateFn('<s:property value="#doc.userId"/>')">
						<img src="http://freedisk.free4lab.com/download?uuid=<s:property value="m[#doc.userId].avatar"/>" border="0" width="40" height="40" onerror="javascript:this.src='images/user_male.png'" />
					</a>
					</s:if>
					<s:else>
						<img src="http://freedisk.free4lab.com/download?uuid=<s:property value="m[#doc.userId].avatar"/>" border="0" width="40" height="40" onerror="javascript:this.src='images/user_male.png'" />
				...
				<td class="leftalign">
					...
					<a id="<s:property value="#doc.userId"/>" href="javascript:void(0)" class="grayletter" onclick="quickCommunicateFn('<s:property value="#doc.userId"/>')">
							<s:property value="m[#doc.userId].userName" />
					</a>
					</s:if>
					<s:else>
						<s:property value="m[#doc.userId].userName" />
					...
					<span class="rightfloat">
						<s:if test="#session.userId==#doc.userId">
							<a href="javascript:void(0)" class="blueletter" onclick="delCmt('<s:property value="#doc.url"/>',this)">删除</a>
						</s:if>
						<s:else>
							<a href="javascript:void(0)" class="blueletter" onclick="replyCmt('<s:property value="#doc.userId"/>','<s:property value = "id"/>','0')">回复</a> 
						</s:else>
					</span> <br />
