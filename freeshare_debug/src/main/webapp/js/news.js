	...
	$.post("ajaxnews", filter, function(data){
		...
		if(news == null){
			...
			for(i = 0; i < news.length; i++){
			    ...
			    if(news[i].type!=0){
			    	if(news[i].type!=-1)
			    		edit = "<a class=\"blueletter leftmargin_5\" target=\"_blank\" href=\"resource/preupdateitem?id="+news[i].resourceId+"\">编辑</a>";
			    	else
				...
				//by wangxy
				//var del = "<a class=\"blueletter leftmargin_5 \" href=\"javascript:del('',"+news[i].resourceId+")\" onclick=\"return confirm('确认删除?');\">删除</a>";
				...
				if(news[i].imgUrl != '' && news[i].imgUrl != null){
					html += "<img src=\""+news[i].imgUrl+"\" border=\"0\" class=\"rightmargin_10\" " +
				...
				//wangxy
				/*if(document.getElementById("hiddenuid").value == news[i].userId)
				{
					html += del;	
				}*/
				//abeibei
...
$("#easynews").click(function(){
	...
	resourceTypeList = "SHARE:ITEM:TOPIC";
	page = 1;
	setFilter(0,0);
	newsFilter(filter, "reload");
	resourceFilterObj.setSelectList(resourceTypeList);
	...
	if(toMySelfCb == 'self1'){
		...
		$.post('upload/publishtopic',{'type':type,'toMySelf':toMySelf,'name':name,'description':description},function(data){
			if(data == undefined){
				...
				//发布按钮置为可用				
				$(obj).val('评论').toggleClass("button graybutton");
...
function updateTopicFn(topicName,topicDesc,topicId){
	//初始化所需数据
	var myurl = "item?id="+topicId,
	myavatar = $("#myOwnAvatarId").val(),
	myname = $("#myOwnNameId").val(),
	myoperation = " 发布了话题 <a href = \""+myurl+"\" class=\"blueletter\">"+topicName+"<\/a>";
	...
	var addToList = "<a class=\"blueletter leftmargin_5\" href=\"resource/preaddtolist?id="+topicId+"&method=newsaddtolist\" rel=\"addtolist\" title=\"选择列表\">加入列表</a>";
	var more = "<a class=\"blueletter leftmargin_5 webwidget_vertical_menu\" href=\"javascript:void(0)\">更多▼</a>"+
				"<ul class=\"webwidget_vertical_menu_content\">"+
