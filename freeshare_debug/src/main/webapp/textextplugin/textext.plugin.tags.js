...
(function($)
{
	...
	p.init = function(core)
	{
		...
		if(self.opts(OPT_ENABLED))
		{
			...
			self.on({
				...
				//下面的这句是我加的,可以将选择框的值转换到。
				
				click           : self.onSetValue,
				//下面的这句是我加的,可以将选择框的值转换到。
				dblclick        :  self.onSetDoubleValue,
	...
	{
		var self       = this,
			inputValue = keyCode === 188 ? '' : self.val(),
		...
		if(keyCode == 188){
			...
			//alert("data"+data[0]);
			//alert("self.val()"+self.val());
		    //alert("formValue"+formValue);
			tag  = self.itemManager().stringToItem(self.val().split(",")[0]);
	...
	//我编写的函数,该函数删除该标签，
	//进行检查标签，如果selectList中有该标签，就删除该标签。
	p.onSetDoubleValue = function(e){
		...
		if(self.isTagAllowed(tag)&& selectList.indexOf(tag) != -1)
		{
			self.removeTag([tag]);
		}
	...
	p.toggleZIndex = function(e)
	{
		var self            = this,
			...
			;
		/*if(!isOnTop){
			//alert("p.toggleZIndex");
			alert("container:"+container); 	
			alert("isOnTop:"+isOnTop); 
		}*/
	...
	p.addTags = function(tags)
	{
		...
		var self      = this,
			...
			;
		//首先是将发送框中的旧的tag删除掉；
		if(typeof(oldSelectList) != "undefined"){
			var tmp  = oldSelectList;
			...
			for(i = 0;i < olds.length - 1;i++){
				self.removeTag(olds[i]);
			}
			oldSelectList = "";
		}
		...
		//然后将新的标签添加到发送框中；同时要添加到selectList中。
		if(typeof(selectList) != "undefined"){
			//alert("defined");
			//alert("selectList3:"+selectList);
			for(i = 0; i < tags.length; i++)
			{
				tag = tags[i];
				if(tag){
	...
	p.removeTag = function(tag)
	{
		...
		if(typeof(selectList) != "undefined"){
			...
			//alert("tmp last:"+tmp[tmp.length-2]);
			/*alert("length:"+tmp[tmp.length-2].length);
			selectList = selectList.substring(0,selectList.length - tmp[tmp.length-2].length-1);*/
			//alert("selectList2:"+selectList);
		}
		self.updateFormCache();
		core.getFormData();
		core.invalidateBounds();
