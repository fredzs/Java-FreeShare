		...
		<div class="bottommargin_5">
	    	<input type="text" id="searchText" style="width:178px;" class="squareinput middleveralign"
	    		onkeydown="if(event.keyCode==13){
					searchList('resource/addtolistsearch',{itemId:<s:property value="id"/>,searchText:$('#searchText').val().trim()},'back');
					return false;}"/>
	    	<a class="sgraybutton" id="searchButton" 
	    		onclick="searchList('resource/addtolistsearch',{itemId:<s:property value="id"/>,searchText:$('#searchText').val().trim()},'back')">群组列表搜索</a>
		</div>
...
function newsaddtolist(){
	var  tipContent = "<table><tr><td><img src='images/success.png'/></td><td>&nbsp;加入列表成功&nbsp;</td></tr></table>";
