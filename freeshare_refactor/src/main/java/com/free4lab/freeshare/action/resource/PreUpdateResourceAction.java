...
public class PreUpdateResourceAction extends BaseAction {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger
			.getLogger(PreUpdateResourceAction.class);
	private Integer id;
	...
	public String pre() {
		...
		List<Integer> idList = ResourcePermissionFactory.getInstance().selectWritableGroups(id);		
		...
		if (wrapper.getType().equals(ResourceTypeConst.TYPE_DOC)) {
			evaluteAttachment(wrapper.getContent());
		} else {
			setAttachment("");
		}
		// 资源id
