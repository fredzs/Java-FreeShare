...
public class SearchAction extends BaseAction {
	...
	public String search() {
		...
		if (null == page)
			this.setPage(1);
		String tags = "";
		logger.info("resourceTypeList:"+resourceTypeList);
		
		if(resourceTypeList.equals(TagValuesConst.FMT_GROUP)){
			tags = TagValuesConst.FMT_GROUP;
		}
		else if (resourceTypeList.equals("all"))
		{
			 String resourceTypeTags = "SHARE:RESOURCE:LIST,SHARE:RESOURCE:TEXT,SHARE:RESOURCE:URL,"
	                  +"SHARE:RESOURCE:VIDEO,SHARE:RESOURCE:DOC,SHARE:RESOURCE:TOPIC,SHARE:FMT:GROUP";
			tags = Tool.returnTags(getSessionUID(), groupIds, resourceTypeTags);
		}
		else 
		{
