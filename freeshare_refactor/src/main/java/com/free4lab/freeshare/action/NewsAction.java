...
public class NewsAction extends BaseAction {
	...
	public String valAvatar() {
		...
		String tonken = getSessionToken();
		userIds.add(userId);
		List<UserInfo> user = UserInfoUtil.getUserInfoByUid(tonken,
				userIds);
		if (user.size() > 0
				...
				for (String id : listIds) {
					try {
						/*Lists lists = new ListHandler().getLists(Integer
								.parseInt(id));*/
						ResourceWrapper resource = new ResourceManager().readResourceWrapper(Integer.parseInt(id));
						if (resource != null) {
							lm.put(id, resource.getName());
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	public String reOperation(String name, String url, Integer optype,
		...
		if (optype != null) {
			...
			case 3: {
				for (String listid : listIds) {
					String listUrl = "resource?id=" + listid;
					Resource l = new ResourceDAO().getResource(Integer
							.parseInt(listid));
