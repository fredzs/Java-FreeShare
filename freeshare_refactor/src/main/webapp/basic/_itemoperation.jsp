...
<div class="bottommargin_5">
	...
	<s:if test="1==permission"><!-- 权限为1表示当前访问者为资源作者，可以修改和删除 -->
		...
		<s:if test="info.type == 1"><!-- 资源类型为1，表示该资源带有附件 -->
			<a class="button"
				href="http://freedisk.free4lab.com/download?uuid=<s:property value="info.content"/>">下载最新版</a>
			<a class="graybutton middlebutton"
				href="resource/newdocversion?id=<s:property value="id" />">发布新版</a>
			<a class="graybutton middlebutton"
				href="resource/preupdateresource?id=<s:property value="id" />">
				编辑</a>
		</s:if>
		<s:else>
		...
		<!-- 删除  -->
		<a class="graybutton middlebutton "
			href="javascript:del('<s:property value="info.type" />')"
