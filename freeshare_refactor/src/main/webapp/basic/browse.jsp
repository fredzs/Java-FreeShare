...
<body>
	...
	//待抽离
	function edit_desc(param,param1,value,uuid){
	...
	if(document.getElementById("edit_val")){
		$("#edit_val").val(value);
		alert('请编辑当前资源后再选择编辑其他资源。');
	}else{
		var html = "<td colspan=\"3\"><textarea class=\"fullarea blue1border\" id=\"edit_val\"></textarea>";
		html += "<input class=\"button rightfloat\" type=\"button\" onclick=\"canceledit('"+param+"')\" value=\"取消\">";
		html += "<input class=\"button rightfloat\" type=\"button\" onclick=\"subedit('"+param+"','"+param1+"','"+uuid+"')\" value=\"确定\">";
		html += "</td>";
		$('#' + param).html(html);
	...
	function canceledit(param){
		$('#'+param).html("");
		$('#'+ param).addClass('hidden');
	}
	function subedit(param,param1, uuid){
		//alert("1:"+$('#name_' + '<s:property value="id" />' + '_0').val());
