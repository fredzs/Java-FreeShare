...
var searchTips = (function($){
	...
	$.fn.searchTips = function(){
		...
		var otherObjs = $(".searchmytypes input[name=types]");
		var resultInput = $("#searchtype");
		var groups = $("#SHARE:ITEM:DOC:1");
		...
		//筛选群组
		$(".searchmytypes input[name=groups]").click(function()
		{
			if($("input[name=groups]").attr("checked") == 'checked'){
				alert("checked!");
			}
			else {
				alert("unchecked!");
