...
var resourceFilterObj = {
		...
		ajaxLoad : function(){
			...
			var url = param.url,
				...
				page = param.page;
				//alert("url:"+url+"paramName:"+paramName+"divPage:"+divPage+"page:"+page);
				//alert(page);
			$.post(url, {
					...
					if (divPage) {
						resourceFilterObj.param.totalNum = data.srhResult.totalNum;
						showDivPage();
					}
					listStyle(data);
				})
		...
		listStyle : function(data){
			...
			var htmlSum = '';
			//alert(data.srhResult.docs.length);
			if(data.srhResult.docs.length > 0){
				var htmlEach = [];
				var docs = data.srhResult.docs;
				for(var i = 0; i < docs.length; i++){
					...
					htmlItem[j++] = '<a class="blackletter" target="_blank"  href="' + docs[i].url + '">';
					htmlItem[j++] = docs[i].title + '</a><br />';
					if(docs[i].description.length > 80){
						docs[i].description = docs[i].description.substring(0, 80)+'...';
					}
					htmlItem[j++] = docs[i].description + '</td><td class="topveralign" width="150px">';
					htmlItem[j++] = docs[i].time + '</td></tr></table></div>';
