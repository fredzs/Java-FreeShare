			 ...
			 function(data) {
				 if(data.groupList.length > 0){
				     ...
				     $.each(data.groupList,function(i,value){
						...
						if((value.groupInfo.hasOwnProperty("avatar")) && (value.groupInfo.avatar!="null")){
							g_avatar = value.groupInfo.avatar;
							
						}
						//判断是否显示默认头像，并设置头像链接
						if (g_avatar == "images/group_head.png") {
							g_src = "images/group_head.png"
						} else {
							g_src = "http://freedisk.free4lab.com/download?uuid="
									+ g_avatar;
						}
						...
						bl_groupsContent += "<div><table class=\"formtable\"><tr><td rowspan=\"2\" width=\"70\"><img src='"
							                + g_src
						...
						if (value.extend == "public") {
							/*bl_groupsContent += "<td class=\"rightfloat\"><a id=\"apply\" href=\"group/resource?groupId="
									+ value.groupId
									+ "&from=register\" onclick = \"goBackRep()\" >加入该组</a></td></tr></table></div>";*/
							bl_groupsContent += "<td class=\"rightfloat\"><a id=\"apply\"  href=\"javascript:join("+value.groupId+")\" " 
									+ ">加入该组</a></td></tr></table></div>";
						} else {
