...
//添加链接的函数
function addlink(){
	var recordUrl = $("#linkId").val().replace(/^\s+|\s+$/g, '');
...
function submiturl(){
	...
	var recordUrl = $("#linkId").val().replace(/^\s+|\s+$/g, '');
	if(recordUrl){
        if(!hrefStartWith(recordUrl,["http","/","ftp://"])) {
        ...
        var delLink = "<span class=\"blueletter\">&nbsp;&nbsp;&nbsp;[&nbsp;<a class=\"blueletter\" href=\"javascript:void(0)\" onclick=\"deleteAddedResource()\">删除</a>&nbsp;]</span>";
        $("#files_1").html("<tr><td>"+recordUrl+delLink + videoTip + "</td></tr>");		
        $("#recordAddedRes").val("<tr><td>"+$("#linkId").val()+delLink + videoTip + "</td></tr>");
        $("#linkId").val(recordUrl);
        hide();
        fillTipBox('success',"链接添加成功！");     
	}else{
		$('#linkText').html('您输入的超链接错误！');
		fillTipBox('error',"链接添加失败！");
		setTimeout("$('#linkText').html('');",3000);
	}
}
//删除“已添加”后边的链接或者文件
...
//检查链接是否包含http等协议名称
$("#linkId").keyup(function(){
	if(!hrefStartWith(this.value,["http","/","ftp://"])){
		$("#linkText").html("您输入的超链接中不包含http等协议名称，默认将为您添加http://前缀");
		setTimeout("$('#linkText').html('');",3000);
	}else{
		$("#linkText").html("");
	}
